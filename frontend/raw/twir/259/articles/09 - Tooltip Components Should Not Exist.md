...
# Item 9: Tooltip Components Should Not Exist

Source: [https://tkdodo.eu/blog/tooltip-components-should-not-exist](https://tkdodo.eu/blog/tooltip-components-should-not-exist)

Summary:
The author argues that low-level <Tooltip> components in design systems are problematic, often leading to accessibility and usability issues. Instead, they advocate for higher-level pattern components (e.g., Button with title, InfoIcon) that enforce consistent, accessible tooltip usage and reduce misuse.

Key takeaways:
- Standalone Tooltip components are prone to misuse and accessibility gaps.
- Prefer higher-level abstractions that integrate tooltips into interactive elements.
- Design systems should restrict low-level tooltip APIs to encourage best practices and consistency.
...
# Tooltip Components Should Not Exist
   ...
   Photo by [Daniel Herron](https://unsplash.com/@herrond)
...
We don’t have to look any further than [Material UI (opens in a new window)](https://mui.com/), one of the most popular component libraries. Their [basic tooltip example (opens in a new window)](https://mui.com/material-ui/react-tooltip/#basic-tooltip) from the docs looks like this:
...
There’s also no way to make this type-safe, as we can’t control the type of children that get passed to the `<Tooltip>` component. [React Aria’s approach (opens in a new window)](https://react-spectrum.adobe.com/react-aria/Tooltip.html) is a bit better, as it won’t show the tooltip at all for non-interactive elements, making it easier to spot for developers who are primarily using their mouse. The fix is then to wrap the [custom trigger (opens in a new window)](https://react-spectrum.adobe.com/react-aria/Tooltip.html#custom-trigger) with the `<Focusable>` component. This is miles better than other approaches, but still suffers from other problems, specifically:
