...
item_type: featured
date: 2025-11-12
source: https://tanstack.com/blog/tanstack-db-0.5-query-driven-sync
tags:
  - "TanStack"
  - "DB"
  - "05"
...
# Item 1: TanStack DB 0.5 — Query-Driven Sync

Source: [https://tanstack.com/blog/tanstack-db-0.5-query-driven-sync](https://tanstack.com/blog/tanstack-db-0.5-query-driven-sync)

Summary:
TanStack DB 0.5 introduces Query-Driven Sync, allowing React components to define their data requirements directly as queries, which are then automatically translated into precise API calls. This approach eliminates the need for custom endpoints or GraphQL resolvers, enabling efficient, live, and incremental data synchronization even with very large datasets. The release adds three sync modes—eager, on-demand, and progressive—so developers can tailor data loading strategies to their application's needs. The update fundamentally changes how data fetching, caching, and live updates are handled in React apps, making data access as declarative as UI rendering.

Key takeaways:
- Query-driven approach: Components declare their data needs as queries, which TanStack DB translates into optimized API calls.
- Three sync modes: Eager (load all), on-demand (load as needed), and progressive (partial load, then background sync) support different data scales and use cases.
- Differential dataflow: Only changed data is recomputed, enabling sub-millisecond updates even with large in-memory datasets.
- Reduces backend/API complexity: No need for custom endpoints or backend changes; queries drive what data is loaded.
...
![Query-Driven Sync](/.netlify/images?url=%2Fblog-assets%2Ftanstack-db-0.5-query-driven-sync%2Fheader.png&w=800&q=80)
