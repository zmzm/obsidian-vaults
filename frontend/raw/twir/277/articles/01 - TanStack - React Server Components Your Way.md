...
# Item 1: TanStack - React Server Components Your Way

Source: [https://tanstack.com/blog/react-server-components](https://tanstack.com/blog/react-server-components)

Summary:
TanStack introduces a new approach to React Server Components (RSCs) that treats them as data streams, giving developers granular control over fetching, caching, and rendering. Instead of enforcing a server-first, framework-owned model, TanStack Start allows RSCs to be used flexibly, similar to fetching JSON, and integrates seamlessly with TanStack Query for cache management. The core APIs are intentionally minimal, focusing on streaming primitives and compatibility with existing tools. This approach simplifies caching and avoids unnecessary framework conventions, making RSCs more composable and easier to adopt incrementally.

Key takeaways:
- RSCs are treated as streams of data, not as a framework-owned tree, enabling client-driven fetching, caching, and composition.
- The API surface is minimal: renderToReadableStream (server), createFromReadableStream (client/SSR), and createFromFetch.
- Caching RSCs is simplified, leveraging existing strategies and infrastructure (memory, CDN, etc.).
- TanStack Query can manage RSC payloads as async resources, requiring no special RSC mode.

Recommendation:
Read fully (for those interested in alternative RSC patterns and practical implementation details)

Why it matters:
for those interested in alternative RSC patterns and practical implementation details
...
![React Server Components](/.netlify/images?url=%2Fblog-assets%2Freact-server-components%2Fheader.jpg&w=800&q=80)
...
Related notes: [[Server Components]], [[TanStack Query]]
