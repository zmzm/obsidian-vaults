...
Summary:
Inngest describes their migration from Next.js to TanStack Start, motivated by the need for faster local development and reduced cognitive overhead. The article details the team's struggles with Next.js's conventions, slow local reloads, and the complexity of RSC and caching APIs. After evaluating several alternatives, they chose TanStack Start for its explicit routing and loader patterns, resulting in dramatically improved developer experience and performance.

Key takeaways:
- Next.js's conventions and RSC complexity can be a barrier for small, full-stack teams not dedicated to frontend work.
- TanStack Start offers explicit routing and loader APIs, reducing magic and improving clarity.
- Local development times dropped from 10–12 seconds to 2–3 seconds, with near-instant route changes after the initial load.
- Migration required some manual effort, especially for shared components and route conversion, but was manageable for a small team.
...
If you're ready to switch to Tanstack Start, the result of our migration is all open source in our UI monorepo: <https://github.com/inngest/inngest/tree/main/ui>.
