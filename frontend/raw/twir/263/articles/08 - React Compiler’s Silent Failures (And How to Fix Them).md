...
Summary:
The author describes how relying on React Compiler for automatic memoization can backfire if the compiler silently fails to optimize a component, leading to performance regressions. To address this, they recommend enabling the undocumented react-hooks/todo ESLint rule to break builds when the compiler can’t process a component, ensuring issues are caught early.

Key takeaways:
- React Compiler removes the need for manual memoization, but silent failures can degrade UX.
- Manual memoization is now considered unnecessary code debt in compiler-enabled projects.
- Enabling the react-hooks/todo ESLint rule surfaces unsupported syntax and prevents silent failures.
- The rule is undocumented but can be configured to enforce strict compiler coverage.
...
## Depending on React Compiler means knowing when it fails
...
](/media/outlyne-typewriter-animation.mp4)
...
](/media/outlyne-typewriter-animation-broken.mp4)
